...
- **iOS（MoriRemote）**：恢复源自 remux 的终端 Dock 布局：Ctrl/Esc/Tab、Sessions/Windows/Panes 和 Keyboard 重新成为三个带原始间距的材质胶囊组。三个导航控件目前均打开既有 Navigator，其动作边界保持独立，留待后续重定义。Navigator 提供明确的主机选择器，只有选择会话后才切换终端；模态表单会暂停终端响应器；中文等输入法的组合文本只会提交一次。
