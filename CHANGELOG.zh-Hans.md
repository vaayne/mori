...
- **macOS**：在创建面板中可直接从开放的 GitHub issue 或 Pull Request 创建工作区——输入 `#` 即可筛选可用键盘导航的 issue/PR 列表（也可粘贴 GitHub issue/PR 链接）。选择 issue 会基于所选基准分支新建分支，自动命名为 `issue-<编号>-<标题-slug>`；选择 PR 则检出该 PR 的头分支，使其徽章与 CI 状态立即生效。
