...
- **macOS**：标题栏标签页不再拉伸占满整个标题栏——单个标签最宽 220pt，过长标题自动截断，单个标签看起来是标签而不是横贯整栏的色带。未选中标签去掉底色（仅保留状态点和标题），悬停时高亮并显示关闭按钮。
...
- **macOS**：标签条过度扩张时，标题栏右侧按钮（文件、Git、分屏、设置）可能永久卡在 `»` 溢出菜单里；现在标签条会主动让出空间，按钮可自行恢复显示。
