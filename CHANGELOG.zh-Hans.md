...
- **iOS（MoriRemote）**：重做终端导航与输入栏：纤薄的四图标键盘附件取代笨重的胶囊 Dock；Navigator 新增明确的主机选择器，只有选择会话后才切换终端；模态表单会暂停终端响应器，服务器与凭据字段可正常输入。Keypad 仍统一修饰键、终端按键和分类的 shell/行编辑快捷键，tmux 继续独占共享变更。
