...
enum SSHPrivateKeyInspectionError: Error, Equatable, LocalizedError, Sendable {
    ...
    var errorDescription: String? {
        switch self {
        case .empty:
            String(localized: "Private key is required.")
        case .tooLarge:
            String(localized: "Private key file is too large.")
        case .invalidOpenSSHPrivateKey:
            String(localized: "Import an OpenSSH private key.")
        case .unsupportedKeyType(let keyType):
            String(
                format: String(localized: "SSH private key type “%@” is not supported."),
                keyType
            )
        }
    }
