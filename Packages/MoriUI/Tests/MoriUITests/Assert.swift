...
/// Approximate equality for floating-point results (luminance, contrast ratios).
func assertApprox(_ a: Double, _ b: Double, tolerance: Double = 1e-6, _ message: String = "", file: String = #file, line: Int = #line) {
    testCount += 1
    if abs(a - b) > tolerance {
        failCount += 1
        print("  FAIL [\(file):\(line)] Expected \(a) ≈ \(b) (±\(tolerance)). \(message)")
    }
}
