...
- **iOS (MoriRemote)**: Added a host-wide agent-attention navigator. It reuses the existing bounded tmux metadata query to rank waiting, working, and completed agents across saved sessions, then safely opens the selected session and pane after its runtime topology arrives.
