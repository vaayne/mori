...
- **macOS**: New local workspaces are now created as APFS copy-on-write clones of the project directory, so `node_modules`, build artifacts, and other untracked files are available instantly. Falls back to `git worktree` (git repos) or a plain copy (non-git) on non-APFS/cross-volume targets. A new setting under Worktree location — "Prefer copy-on-write clones for new workspaces" — toggles the behavior (on by default). Clones on disk are auto-discovered at launch, and deleting a clone warns before destroying unpushed local commits.
...
- **macOS**: ⌘1–9 quick jump now selects workspaces (worktrees) instead of individual tmux windows, matching the sidebar hints.
- **macOS**: The $HOME workspace is pinned as a single "Home" row at the top of the sidebar — one click drops into a session at $HOME for tasks that don't belong to any repository. It has no workspace list or "+ New workspace" row and is excluded from ⌘1–9 indexing.
- **macOS**: Restored the agent summary strip above the sidebar footer — waiting / running / error dots with counts, dimmed when zero. Clicking a state lists its workspaces — pick one to jump straight to it.
...
- **macOS**: Simplified the sidebar chrome by removing the redundant Projects add button and inactive search glyph.
...
- **macOS**: Fixed a v0.5.2 regression where applying split light/dark themes via libghostty color-scheme mutation could interfere with new terminal surface startup and prevent zsh startup files from being loaded reliably.

**Full Changelog**: [v0.5.2...v0.5.3](https://github.com/vaayne/mori/compare/v0.5.2...v0.5.3)
...
- **macOS**: The sidebar background is now pushed one step back from the terminal canvas (darkened ~22% in dark themes, ~6% in light), so the chrome reads as its own plane instead of dissolving into the terminal when both follow the Ghostty theme. The tint is derived from the synced theme colour, so the Ghostty-sync convention is preserved.
- **macOS**: Sidebar polish pass — filter strip now reads as text (`1 waiting`, `0 running`) with status dots and a search affordance, and zero-count states hide so the strip stays quiet when nothing is happening. Project headers are pulled back up to a real header weight (20pt tile + 14pt bold name) so grouping is unmistakable, while still letting the selected worktree row keep the visual lead. tmux window rows switched to the same glyph language as worktree rows — a small SF Symbol tinted by agent state (pulsing while waiting) instead of an opaque dot — so the two levels read as related.
...
**Full Changelog**: [v0.4.7...v0.4.8](https://github.com/vaayne/mori/compare/v0.4.7...v0.4.8)
...
**Full Changelog**: [v0.4.3...v0.4.4](https://github.com/vaayne/mori/compare/v0.4.3...v0.4.4)
...
**Full Changelog**: [v0.4.2...v0.4.3](https://github.com/vaayne/mori/compare/v0.4.2...v0.4.3)
