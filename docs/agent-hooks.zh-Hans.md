...
- `@mori-agent-state`：`"working"`（代理运行中）、`"waiting"`（一轮结束，等待用户输入）或 `"done"`（代理已退出）
- `@mori-agent-name`：代理名称（例如 `"claude"`、`"codex"`、`"pi"`）

在 Mori 的 5 秒轮询周期中，它直接从 tmux 读取这些选项——无需进程扫描，无需模式匹配。标签页名称会显示代理名称（如 `claude`、`codex`、`pi`），侧边栏显示徽章（⚡ 运行中 / ❗ 等待中 / ✅ 已完成）。状态切换时会发送 macOS 通知。
...
在 tmux 窗口中运行 Codex。运行期间标签页会重命名为 `codex`（侧边栏显示 ⚡ 徽章），一轮完成后显示等待徽章（❗）。
...
3. Mori 会在 `~/.config/mori/mori-pi-extension.ts` 创建 TypeScript 扩展，并在 `~/.pi/agent/settings.json` 中注册：
   ...
   ```
4. 后续每次启动 Mori 时，如果扩展文件缺失或已过期，都会从当前 bundle 重新复制。

在 tmux 窗口中运行 Pi。运行期间标签页会重命名为 `pi`（侧边栏显示 ⚡ 徽章），一轮完成后显示等待徽章（❗）。
